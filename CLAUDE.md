...
## Dataset scuole secondarie (condiviso tra /registrazione e /per-le-scuole)
...
- Il menu a cascata provincia → indirizzo → scuola è incapsulato nel componente condiviso `components/ScuolaCascadeFields.tsx` (legge il JSON via `fetch` client-side, non è importato nel bundle JS per non appesantirlo), riusato sia dal form studenti (`RegistrazioneForm`) sia dal form lead scuole (`RichiestaInformazioniForm`) — non duplicare questa logica altrove
